...
# task's row from tasks.tsv, then hands off to job_inner.sh inside cmssw-el9.
#
# Env in (exported by the generated job.sh):
#   HZZ_INNER HZZ_CMSSW_TGZ HZZ_TASKS HZZ_PROXY HZZ_BIND
#   [HZZ_ENTRIES] [HZZ_CHANNELS] [HZZ_SYST]
set -uo pipefail
...
exec /cvmfs/cms.cern.ch/common/cmssw-el9 -B "${HZZ_BIND:?}" -- bash "${HZZ_INNER:?}"
